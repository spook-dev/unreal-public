...
bool UIpConnection::SetTOS(uint8 TOS)
{
	if (Driver->ServerConnection != this) {
		return Driver->ServerConnection->SetTOS(TOS);
	}
	else {
		SOCKET socket = Socket.GetNativeSocket();
		return Socket->SetTOS(TOS);
	}
}
