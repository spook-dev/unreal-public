...
bool FSocketBSD::Connect(const FInternetAddr& Addr)
{
	...
	const FInternetAddrBSD& BSDAddr = static_cast<const FInternetAddrBSD&>(Addr);
	//ConnectedAddr = (PSOCKADDR)(const sockaddr*)&(BSDAddr.Addr);
...
bool FSocketBSD::SendTo(const uint8* Data, int32 Count, int32& BytesSent, const FInternetAddr& Destination)
{
	...
	ConnectedAddr = (PSOCKADDR)&(BSDAddr.Addr);
	//UE_LOG(LogTemp, Display, TEXT("FSocketBSD::SendTo on socket %X"), Socket);
...
bool FSocketBSD::SetTOS(uint8 TOS)
{
	//DSCP is the top 6 bits of the 8 bits of TOS
	uint32 DSCP = TOS / 4;
#if PLATFORM_HAS_BSD_SOCKET_FEATURE_WINSOCKETS
	UE_LOG(LogTemp, Display, TEXT("FSocketBSD::SetTOS Setting TOS with Winsock2"));
	//Init vars for Qwave API calls
	QOS_VERSION QosVersion = { 1 , 0 };
	HANDLE qosHandle;
	//flowID should be 0 when passed in, and will contain a given ID after QOSAddSocketToFlow
	QOS_FLOWID flowId = 0;

	if (QOSCreateHandle(&QosVersion, &qosHandle) == false) {
		UE_LOG(LogTemp, Display, TEXT("FSocketBSD::SetTOS QOSCreateHandle failed, error code %d\n"), WSAGetLastError());
		return false; //return GetLastError();
	}

	//Set nearest non-privileged codepoint
	//reference: https://docs.microsoft.com/en-us/windows/win32/api/qos2/ne-qos2-qos_traffic_type
	//note that 802.1p tags will not be applied unless QOS_NON_ADAPTIVE_FLOW is not set when calling QOSAddSocketToFlow
	//CS0
	QOS_TRAFFIC_TYPE TrafficType;
	if (TOS < 0x20){ //no change. CS0
	...
	//CS5
		...
		//QosTrafficTypeExcellentEffort is identical in terms of DSCP and 802.1p tag, but lower priority in winsock
		TrafficType = QOSTrafficTypeAudioVideo;
	}
	//CS7
	else {
		//QosTrafficTypeControl is identical in terms of DSCP and 802.1p tag, but has higher priority in winsock
		TrafficType = QOSTrafficTypeVoice;
	}

	if (QOSAddSocketToFlow(qosHandle, Socket, ConnectedAddr, TrafficType, QOS_NON_ADAPTIVE_FLOW, &flowId) == false) {
		UE_LOG(LogTemp, Display, TEXT("FSocketBSD::SetTOS QOSAddSocketToFlow failed, error code %d\n"), WSAGetLastError());
		return false; //return GetLastError();
	}
	//then attempt to set exact desired codepoint
	if (QOSSetFlow(qosHandle, flowId, QOSSetOutgoingDSCPValue, sizeof(DSCP), (char*)&DSCP, 0, NULL) == false) {
		UE_LOG(LogTemp, Display, TEXT("FSocketBSD::SetTOS QOSSetFlow failed, error code %d\n"), WSAGetLastError());
		return false; //return GetLastError();
	}
	return true;
#else
	return setsockopt(Socket, IPPROTO_IP, IP_TOS, &DSCP, sizeof(DSCP));
#endif
}
